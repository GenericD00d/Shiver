...
- **mobile/bridge**: `index.ts` (install, `seedSession`), `home.ts` (`setHome`, `goHome`, `installHomeSwipe`: back and a
  ...
  channel menu with mark all read: `installChannelMenu`, `closeChannelMenu`; `drawerIsOpen`, `openConversation`), `reconnect.ts` (`installAutoReconnect`; meanwhile a frozen copy of the page taken as the app went to the background, with an accent spinner above the chat box), `document-start.ts` (seed,
