 ...
 * stays Sharkord's. On a page without a sidebar (sign-in, errors) any right swipe goes home.
 */
export function installHomeSwipe() {
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      if (!event.isTrusted || dx < SWIPE_THRESHOLD || dx <= Math.abs(lastY - startY)) return;
